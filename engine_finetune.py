...
import pandas as pd
from util.datasets import denormalize_age
...
def evaluate(data_loader, model, device, args, epoch, mode, log_writer):
    ...
    preds = denormalize_age(np.array(preds))
    trues = denormalize_age(np.array(trues))
    ...
    if args.dataset_name != 'alzeye':
        results_path = os.path.join(args.output_dir, args.task, f'metrics_{args.dataset_name}.csv')
    ...
    if args.dataset_name != 'alzeye':
        output_csv_path = os.path.join(args.output_dir, args.task, f'Predictions_{args.dataset_name}.csv')
