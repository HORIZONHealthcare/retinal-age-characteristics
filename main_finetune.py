...
from models_vit import RETFound_dinov2, RETFound_dinov2_MM
...
def get_args_parser():
    ...
    parser.add_argument('--load_retfound', action='store_true', default=False)
    parser.add_argument('--mm_model', action='store_true', default=False)
    parser.add_argument('--enable_projector', action='store_true', default=False)
    parser.add_argument('--train_mode', default='finetune', type=str)
    parser.add_argument('--modalities', default='CFP', type=str)
    parser.add_argument('--patient_condition', default='mixed', type=str)
    parser.add_argument('--dataset_name', default='alzeye', type=str)
...
def main(args, criterion):
    ...
    if args.mm_model:
        model = RETFound_dinov2_MM(n_modalities=len(args.modalities), enable_projector=args.enable_projector)
    else:
        model = RETFound_dinov2()
    
    # '''
    if args.load_retfound: # args.finetune and not args.eval:
        ...
        if args.mm_model:
            state_dict = model.backbone_list[0].state_dict()
            ...
            interpolate_pos_embed(model.backbone_list[0], checkpoint_model)
            for i in range(len(model.backbone_list)):
                msg = model.backbone_list[i].load_state_dict(deepcopy(checkpoint_model), strict=False)
        else:
    ...
    eff_batch_size = args.batch_size * args.accum_iter * misc.get_world_size() # 32
    ...
    if args.train_mode == 'finetune':
        ...
        if args.mm_model:
            param_groups = []
            for i in range(len(model_without_ddp.backbone_list)):
                if i == 0:
                    head = model_without_ddp.linear
                    if args.enable_projector:
                        projector = model_without_ddp.projector_list
                    else: projector = None
                else: 
                    head = None
                    projector = None
                param_groups.extend(lrd.param_groups_lrd(model_without_ddp.backbone_list[i], head=head, projector=projector, weight_decay=args.weight_decay,
                                                        no_weight_decay_list=no_weight_decay,
                                                        layer_decay=args.layer_decay
                                                        ))
        else:
            param_groups = lrd.param_groups_lrd(model_without_ddp, weight_decay=args.weight_decay,
                                                no_weight_decay_list=no_weight_decay,
                                                layer_decay=args.layer_decay
                                                )
    elif args.train_mode == 'linear_probe':
        assert args.mm_model
        # model_without_ddp.freeze_backbones()
        param_groups = model_without_ddp.linear.parameters()
    ...
    if args.eval:
        ...
        evaluate(data_loader_test, model, device, args, epoch=0, mode='test', log_writer=log_writer)
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == (args.epochs - 1):
            ...
            evaluate(data_loader_test, model, device, args, -1, mode='test', log_writer=None)
